...
public interface StudentRepo extends JpaRepository<Students, Integer> {
	...
	//not for me
	@Query("select new com.ucsy.ams.dto.StudentAccountDto(s.id,a.name,a.email,s.rollNo, s.nrcNo, s.dob,"
			+ "s.phoneNo,s.major, s.registerDate, s.semester) " + "from Students s join Account a on s.id = a.id "
			+ "join Takes t on t.student.id = s.id " + "join Courses c on t.course.id = c.id where c.id = ?1")
