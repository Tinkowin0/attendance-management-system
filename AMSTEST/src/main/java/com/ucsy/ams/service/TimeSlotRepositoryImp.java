...
public class TimeSlotRepositoryImp implements TimeSlotRepository {
    ...
    @Override
    public List<GetTimeSlotBySemesterIdDTO> findTimeSlotForTeacher(int teacherId) {
    	
        String sql = " SELECT day_of_week, start_time, end_time, code as course_code, teaches_id FROM timeslot " +
        ...
        return jdbcTemplate.query(sql, new Object[]{teacherId}, new TimeSlotRepositoryImp.TimeSlotRowMapper());
    }
    ...
    private static class TimeSlotRowMapper implements RowMapper<GetTimeSlotBySemesterIdDTO> {
        ...
        public GetTimeSlotBySemesterIdDTO mapRow(ResultSet rs, int rowNum) throws SQLException {
            ...
            return new GetTimeSlotBySemesterIdDTO(day_of_week, start_time, end_time, course_code, teaches_id);
        }
    }

    @Override
    public List<GetTimeSlotBySemesterIdDTO> findTimeSlotForStudent(String semesterName, int studentId) {
        String sql = "SELECT day_of_week, start_time, end_time,code as course_code,course_name, teaches_id,account.name FROM timeslot " +
                "LEFT JOIN semester on timeslot.semester_id = semester.id " +
                "LEFT JOIN courses on timeslot.courses_id = courses.id " +
    ...
    private static class TimeSlotStudentRowMapper implements RowMapper<GetTimeSlotBySemesterIdDTO> {
        ...
        public GetTimeSlotBySemesterIdDTO mapRow(ResultSet rs, int rowNum) throws SQLException {
            ...
            return new GetTimeSlotBySemesterIdDTO(day_of_week, start_time, end_time, course_code, teaches_id,  teacher_name, course_name);
        }
    }
}
